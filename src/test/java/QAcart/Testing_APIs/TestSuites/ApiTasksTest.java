...
public class ApiTasksTest {
    ...
    //Delete requests
    @Test
    @Description("Deletes a task successfully")
    public void DeleteTask(){
        TaskPojo taskPojo = TaskBuild.CreateNew();
        String token = UserBuild.CreateToken();
        String taskID = TaskBuild.GetID(taskPojo, token);
        Response response = TasksApi.DeleteTask(token, taskID);
        TaskPojo resTask = response.body().as(TaskPojo.class);
        assertThat(response.statusCode(), equalTo(200));
        assertThat(resTask.getId(), equalTo(taskID));
    }

    @Test
    @Description("No tasks should be deleted without a token")
    public void InvalidDeleteTask(){
        TaskPojo taskPojo = TaskBuild.CreateNew();
        String token = UserBuild.CreateToken();
        String taskID = TaskBuild.GetID(taskPojo, token);
        token = null;
    ...
    public void UpdateTask(){
        ...
        String token = UserBuild.CreateToken();
        String taskID = TaskBuild.GetID(taskPojo, token);
        taskPojo.setisCompleted(true);
        Response response = TasksApi.UpdateTask(taskPojo, token, taskID);
        TaskPojo resTask = response.body().as(TaskPojo.class);
        assertThat(response.statusCode(), equalTo(200));
        assertThat(resTask.getId(), equalTo(taskID));
        assertThat(resTask.getisCompleted(), equalTo(true));
    }
}
